...
5. If you change the definition of `NDIRECT`, make sure to create a new `fs.img`, since mkfs uses `NDIRECT` to build the file system.
6. If your file system gets into a bad state, perhaps by crashing, delete `fs.img` (do this from Unix, not xv6). `make` will build a new clean file system image for you.
7. Don't forget to `brelse()` each block that you `bread()`.
8.  You should allocate indirect blocks and doubly-indirect blocks only as needed, like the original `bmap()`.
9.  Make sure `itrunc()` frees all blocks of a file, including double-indirect blocks.
10. You can pass by only modifying:
    -  `fs.c`
    -  `fs.h`
    -  `file.h`
...
### 3. Symbolic Link to Directories (2 points)
#### Description
...
### 4. Report (1 point)
Your report should include the following two points:
1. Briefly explain how you solve each problem above. Note that even if you fail on solving the problems, you still can get one point if your reports have good solutions (for all of them).
...
The report should follow these format:
   - Should be a PDF.
   - Do not exceed two pages for the first part, while there is no limitation for the second part.
 
## Submit Rules
- Submit `xv6/` to github classroom.
  - Do not submit files generated by `Makefile`. You can `make clean` before you submit. 
- Submit report to gradescope.

## Grading Policy


How we run your code:
- We will use the original `Makefile`. Make sure your implementation can be configured by it.
- We will slightly modify the output of `bigfile.c` and `symlinktest.c` to prevent naive printing.
- Three problems will use the same code, so you should handle them at the same time.
...
- You will get all points for each subproblems if you pass the testing commands. Otherwise, you don't get any point.
- For example, you pass **Large Files** and **Symbolic links to Files**, but you fail on **Symbolic links to Directories**. You still hand in a complete **Report**. In this case, you get 9 points.
